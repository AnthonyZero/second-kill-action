...
public class GenerateUserUtil {
	...
	/**
	 * 生成count个用户信息 -》 存于数据库 并执行登录获取token -》存于文件
	 * @param count
	 * @throws Exception
	 */
	private static void createUser(int count) throws Exception {
		...
		for (int i = 0; i < count; i++) {
			...
			user.setSalt("anthonyzero");
			user.setPassword(Md5Util.getMd5(Md5Util.inputPassEncrypt("123456"), user.getSalt()));
			users.add(user);
		}
		...
		for (int i = 0; i < users.size(); i++) {
			...
			String params = "mobile=" + user.getId() + "&password=" + Md5Util.inputPassEncrypt("123456");
